...
public class TimeManagerDaoImpl extends BaseDaoImpl implements TimeManagerDao {
    ...
    @Override
    public List<TimeManagerPlanItem> findAll(User user, TimeManagerItem item) throws ConnectionPoolException, DBException {
        Connection connection = initConnection();
        ResultSet set;
        PreparedStatement statement;
        try {
            statement = connection.prepareStatement(SELECT_ALL_TIME_MANAGE);
            statement.setInt(1, user.getId());
    ...
    private void insert(TimeManagerPlanItem item, Connection connection) throws DBException {
        try {
            ...
            statement.setString(4, item.getTimeEnd());
            statement.executeUpdate();
        } catch (SQLException e) {
            throw new DBException(e);
        }
