...
public class CashFlowDaoImpl extends BaseDaoImpl implements CashFlowDao {
    ...
    @Override
    public List<CashFlowPlanItem> findAll(User user, CashFlowItem item) throws ConnectionPoolException, DBException {
        Connection connection = initConnection();
        ResultSet set;
        try {
            PreparedStatement statement = connection.prepareStatement(SELECT_ALL_CASH_FLOW);
            statement.setInt(1, user.getId());
    ...
    private List<CashFlowPlanItem> parseSet(ResultSet set, CashFlowItem item, User user) throws DBException {
        List<CashFlowPlanItem> items = new ArrayList<>();
        try {
