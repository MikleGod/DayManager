function ajaxCalendar(year, month, day) {
    month++;
    let strMonth;
    let strDay;
    if (month < 10) {
    ...
    $.ajax({
        url: "/controller",
        type: "POST",
        data: "action=calendar&date="+date,
...
function tmi_cfi(id, id2) {
    if (document.getElementById('cf').style.display === 'none') {
        document.getElementById('cf').style.display = 'block';
        document.getElementById('tm').style.display = 'none';
    }
}

function cfi_tmi(id, id2) {
    ...
    for (var i = 1; i <= Dlast; i++) {
        if (i == new Date().getDate() && D.getFullYear() == new Date().getFullYear() && D.getMonth() == new Date().getMonth()) {
            ...
            (i == 4 && D.getMonth() == 10 && D.getFullYear() > 2004) // День народного единства, который заменил Октябрьскую революцию 1917 года
                ...
                calendar += '<td onclick="ajaxCalendar(' + year + ', ' + month + ', ' + i + ')" class="holiday">' + i;
            } else {
                calendar += '<td onclick="ajaxCalendar(' + year + ', ' + month + ', ' + i + ')">' + i;
            }
        }
